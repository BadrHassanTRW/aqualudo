(function (global) {
  ...
  const isAdminPath = () => {
    ...
    return path.includes('/admin/') || path.endsWith('/admin') || /\/admin(\/|$|\.)/.test(path);
  };
  const isPublicPath = () => !isAdminPath();
  const rootHome = () => BASE + 'index.html';
  const pagesBase = () => BASE + 'site/pages/';
  const adminBase = () => BASE + 'site/admin/';

  // ===========================
  // PUBLIC HEADER (with search)
  // ===========================
  function renderHeader(active) {
    ...
    const userLabel = customer ? (customer.name.split(' ')[0]) : 'حسابي';
    return `
  ...
  // ===========================
  // PUBLIC FOOTER (with quick action bar)
  // ===========================
  ...
  function admPageTitle(p) {
    if (p === 'index.html') return 'نظرة عامة';
    if (p === 'bookings.html') return 'الحجوزات';
    if (p === 'activities.html') return 'الأنشطة';
    if (p === 'contacts.html') return 'رسايل الاتصال';
    if (p === 'login.html') return 'دخول الموظف';
    return 'لوحة التحكم';
  }
  ...
  // ===========================
  // Bind header (search + user menu + mobile menu)
  // ===========================
  function bindHeader() {
    const header = $('#header');
    if (header) {
  ...
  function submitQuickBook() {
    if (!qbState.activityId || !qbState.sessionId) { toast('اختار نشاط وموعد', 'error'); return; }
    if (!qbState.name.trim() || !qbState.phone.trim()) { toast('دخّل اسمك ورقمك', 'error'); return; }
    const allSessions = AquaDB.sessions.upcoming();
    const session = allSessions.find(s => s.id === qbState.sessionId);
    if (!session) { toast('الموعد مش متاح', 'error'); return; }
    ...
    closeQuickBook();
    toast('استلمنا طلبك، هنكلمك على واتساب', 'success');
  ...
  function openActivityModal(slug) {
    ...
    if (!a) { toast('النشاط مش موجود', 'error'); return; }
    const m = $('.modal', modal);
    m.innerHTML = `
      <div class="modal-head">
        <h2>${a.name}</h2>
  ...
  function buildCmdkData() {
    ...
    const nav = isAdmin ? [
      { kind: 'إجراء', title: 'الرئيسية', sub: 'نظرة عامة على الأكاديمية', href: adminBase() + 'index.html', icon: 'home' },
      { kind: 'إجراء', title: 'الحجوزات', sub: 'كل حجوزات العملاء', href: adminBase() + 'bookings.html', icon: 'calendar' },
      { kind: 'إجراء', title: 'الأنشطة', sub: 'إدارة الأنشطة والأسعار', href: adminBase() + 'activities.html', icon: 'activity' },
      { kind: 'إجراء', title: 'الرسايل', sub: 'رسايل نموذج التواصل', href: adminBase() + 'contacts.html', icon: 'mail' },
      { kind: 'إجراء', title: 'إضافة حجز', sub: 'حجز جديد من الموظف', action: 'qb', icon: 'plus' },
      { kind: 'إجراء', title: 'تسجيل خروج', sub: 'خروج من لوحة التحكم', action: 'logout', icon: 'logout' },
    ] : [
      { kind: 'صفحة', title: 'الرئيسية', sub: 'صفحة الهبوط', href: rootHome(), icon: 'home' },
      { kind: 'صفحة', title: 'الأنشطة', sub: 'كل الرياضات المائية', href: pagesBase() + 'activities.html', icon: 'activity' },
      { kind: 'صفحة', title: 'الأسعار', sub: 'الباكدجز والـ memberships', href: pagesBase() + 'pricing.html', icon: 'tag' },
      { kind: 'صفحة', title: 'الإيفنتات', sub: 'إيفنتات جاية', href: pagesBase() + 'events.html', icon: 'star' },
      { kind: 'صفحة', title: 'مين احنا', sub: 'قصة الأكاديمية', href: pagesBase() + 'about.html', icon: 'info' },
      { kind: 'صفحة', title: 'تواصل', sub: 'كل القنوات', href: pagesBase() + 'contact.html', icon: 'phone' },
      { kind: 'صفحة', title: 'حسابي', sub: 'حجوزاتي وبروفايلي', href: pagesBase() + 'account.html', icon: 'user' },
      { kind: 'صفحة', title: 'لوحة التحكم', sub: 'دخول الموظف', href: adminBase() + 'login.html', icon: 'lock' },
      { kind: 'إجراء', title: 'احجز سيشن', sub: 'حجز سريع في نافذة', action: 'qb', icon: 'plus' },
    ];

    const items = [
      ...nav,
      ...acts.filter(a => a.active).map(a => ({
        kind: 'نشاط', title: a.name, sub: a.tagline,
        href: pagesBase() + 'activity.html?slug=' + a.slug,
        action: a.slug, icon: 'activity',
      })),
      ...events.map(e => ({
        kind: 'إيفنت', title: e.title, sub: e.tagline,
        href: pagesBase() + 'event.html?slug=' + e.slug,
        icon: 'star',
      })),
      ...pkgs.map(p => ({
        kind: 'باكدج', title: p.name, sub: p.desc + ' · ' + p.price.toLocaleString() + ' ج',
        href: pagesBase() + 'pricing.html#' + p.id,
        icon: 'tag',
      })),
      ...mems.map(m => ({
        kind: 'Membership', title: m.name, sub: m.desc + ' · ' + m.price.toLocaleString() + ' ج',
        href: pagesBase() + 'pricing.html#' + m.id,
        icon: 'tag',
      })),
      ...sessions.slice(0, 8).map(s => {
        ...
        const time = d.toLocaleTimeString('ar-EG', { hour: 'numeric', minute: '2-digit' });
        return {
          kind: 'موعد', title: (a ? a.name : 'سيشن') + ' · ' + day + ' ' + d.getDate() + ' · ' + time,
          sub: s.booked + '/' + s.capacity + ' محجوز',
          action: 'qbSession', id: s.id, activityId: s.activityId,
          icon: 'clock',
        };
      }),
    ];
  ...
  function renderCmdkResults(q) {
    ...
    if (list.length === 0) {
      res.innerHTML = `<div class="cmdk-empty">مفيش نتائج لـ "<strong>${q}</strong>". جرّب كلمة تانية.</div>`;
      return;
    }
    const byKind = {};
    ...
    // keep flat list in sync for keyboard nav
    res._flat = list;
  }
  ...
  // ===========================
  // Bind everything (public or admin)
  // ===========================
  function bindGlobal() {
    // Cmd+K
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        const qb = $('#qbModal'); if (qb && qb.classList.contains('open')) { closeQuickBook(); return; }
        const am = $('#activityModal'); if (am && am.classList.contains('open')) { closeActivityModal(); return; }
        const sh = $('#shortcutOverlay'); if (sh && sh.classList.contains('open')) { closeShortcuts(); return; }
      }
      if (isOpen) {
      ...
      if (e.key === 'x' || e.key === 'X') {
        if (isAdminPath() && location.pathname.includes('bookings')) {
          // focus first "confirm" button if present
    ...
    // Auto-wire any data-fab="book" / data-qb="1" elements (bottom nav button)
    $$('[data-qb]').forEach(b => b.addEventListener('click', (e) => { e.preventDefault(); openQuickBook(); }));
  }

  function openShortcuts() { const sh = $('#shortcutOverlay'); if (sh) sh.classList.add('open'); }
  function closeShortcuts() { const sh = $('#shortcutOverlay'); if (sh) sh.classList.remove('open'); }

  // ===========================
  // Activity cards: add quick-view button + delegate clicks
  // ===========================
  ...
  // ===========================
  // init: apply shell class to body
  // ===========================
